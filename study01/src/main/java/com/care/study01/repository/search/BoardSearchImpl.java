...
import com.care.study01.domain.QReply;
...
//        query.where(board.title.contains("1")); //where title like..
    ...
    public Page<Board> searchAll(String[] types, String keyword, Pageable pageable) {
        ...
        List<Board> list = query.fetch();
        long count = query.fetchCount();
        return new PageImpl<>(list, pageable, count); // 인수를 넘김
    ...
    public Page<BoardListReplyCountDTO> searchWithReplyCount(String[] types, String keyword, Pageable pageable) {
        ...
        // reply의 board_bno와 board의 bno JOIN -> reply 값을 기준으로

        query.groupBy(board);
        //select 를 이용하여 Proejections.bean() 활용하기
        ...
        // bno > 0
        query.where(board.bno.gt(0L)); // and bno greater 0

        //원하는 내용만 조회(전체가 아님)
        JPQLQuery<BoardListReplyCountDTO> dtoQuery = query.select(Projections.bean(BoardListReplyCountDTO.class,
                board.bno, board.title, board.writer, board.regDate, reply.count().as("replyCount")));

        this.getQuerydsl().applyPagination(pageable, dtoQuery); //페이지 목록화
